...
/**
 * @brief Fast atomic max for float using CAS
...
__device__ inline float atomicMaxFloat(float* addr, float value) {
    ...
    // Early exit if current value is already larger
    if (__uint_as_float(old) >= value) return __uint_as_float(old);
    
    do {
        assumed = old;
        old = atomicCAS(addr_as_uint, assumed, __float_as_uint(value));
    } while (assumed != old && __uint_as_float(old) < value);
