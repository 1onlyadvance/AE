...
#include "neuromorphic/include/spike_encoder.h"
...
public:
    AresEdgeSystem() : running_(false), active_threats_(0), active_nodes_(0), consensus_health_(0.0f), prediction_accuracy_(0.0f), mission_time_ms_(0) {}
    ...
    bool initialize() {
        try {
            ...
            if (cew_module_->initialize(0) != cudaSuccess) { // Pass device_id
                return false;
            }

            // Initialize Neuromorphic
            loihi_ = std::make_unique<neuromorphic::Loihi2HardwareAbstraction>();
            // Use correct initialization signature for Loihi2HardwareAbstraction
            if (loihi_->initialize(neuromorphic::ExecutionMode::GPU_SIMULATION, 0, nullptr) != cudaSuccess) {
                return false;
            }

            spike_encoder_ = std::make_unique<neuromorphic::SpikeEncoder>();
            // No initialize() method for SpikeEncoder, so skip

            // Initialize Swarm
            // Provide required arguments to constructors
            consensus_ = std::make_unique<swarm::ByzantineConsensusEngine>(0, 50); // node_id=0, swarm_size=50
            std::vector<swarm::NodeInfo> initial_nodes(50); // Dummy node info
            if (consensus_->initialize(initial_nodes) != cudaSuccess) {
                return false;
            }

            task_auction_ = std::make_unique<swarm::DistributedTaskAuction>(0, 50); // agent_id=0, swarm_size=50
            std::vector<swarm::AgentCapabilities> agent_caps(50); // Dummy agent capabilities
            if (task_auction_->initialize(agent_caps) != cudaSuccess) {
                return false;
            }
            ...
            sim_params.timestep_s = 0.001f;
            sim_params.substeps = 1;
            sim_params.max_velocity = 100.0f;
            sim_params.collision_margin = 0.01f;
